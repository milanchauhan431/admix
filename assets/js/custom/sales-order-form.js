...
function createOrder(){
	var fromEntryTypes = $("#saveSalesOrder #from_entry_type").val();
	var refIds = $("#saveSalesOrder #ref_id").val();
...
function AddRow(data) {
	...
	/* cell = $(row.insertCell(-1));
	var filePostData = {"index":itemCount,"inputName":"itemData["+itemCount+"][attachment]","inputStatus":"itemData["+itemCount+"][attachment_status]","file":((data.attachment != "")?base_url+"assets/uploads/sales_order/"+data.attachment:""),"fileName":((data.attachment != "")?data.attachment:"")};
	cell.html(attachmentInput(filePostData));
	cell.append('<div class="error ba_file_'+itemCount+'"></div>');
