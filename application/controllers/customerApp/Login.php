...
class Login extends CI_Controller{
	...
	public function index(){	
		if($this->session->userdata('LoginOk')):
			redirect( base_url('customerApp/dashboard') );
		else:
			$this->load->view('customer_app/login');
		endif;
	}
	...
	public function auth(){
		if($this->session->userdata('LoginOk')):
			...
			if($this->form_validation->run() == true):
				...
				if($result['status'] == 1):
					return redirect( base_url('customerApp/dashboard') );
				else:
					$this->session->set_flashdata('loginError',$result['message']);
					redirect( base_url('customerApp/login') , 'refresh');
				endif;
			else:
				$this->load->view('customer_app/login');
			endif;
	  ...
	  function logout(){
		...
		return redirect(base_url('customerApp/login'));
	}
