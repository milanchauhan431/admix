...
class GstExpenseModel extends MasterModel{
    ...
    public function save($data){
        try{
            ...
            if(!empty($data['id'])):
                ...
                $this->remove($this->transDetails,['main_ref_id'=>$data['id'],'table_name'=>$this->transMain,'description'=>"GEXP TERMS"]);
                $this->remove($this->transDetails,['main_ref_id'=>$data['id'],'table_name'=>$this->transMain,'description'=>"GEXP MASTER DETAILS"]);
            endif;
    ...
    public function getGstExpense($data){
        $queryData = array();
        $queryData['tableName'] = $this->transMain;
        ...
        if($data['itemList'] == 1):
            $result->itemList = $this->getGstExpenseItems($data);
        endif;
        ...
        $queryData['where']['description'] = "GEXP TERMS";
        $result->termsConditions = $this->rows($queryData);

        return $result;
    }

    public function getGstExpenseItems($data){
        $queryData = array();
    ...
    public function getGstExpenseItem($data){
        $queryData = array();
        $queryData['tableName'] = $this->transChild;
        $queryData['where']['id'] = $data['id'];
        $result = $this->row($queryData);
    ...
    public function delete($id){
        try{
            ...
            $this->remove($this->transDetails,['main_ref_id'=>$id,'table_name'=>$this->transMain,'description'=>"GEXP TERMS"]);
            $this->remove($this->transDetails,['main_ref_id'=>$id,'table_name'=>$this->transMain,'description'=>"GEXP MASTER DETAILS"]);
